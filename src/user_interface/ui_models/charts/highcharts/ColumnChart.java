...
public class ColumnChart extends AbstractChart {
    public ColumnChart(String title, List<Series<?>> data, Double min, Double max, String xAxisSuffix, String yAxisSuffix, String xLabel, String yLabel, int xDecimals, int yDecimals, List<String> categories) {
        ...
        options=new Options()
                ...
                .setTooltip(new Tooltip().setEnabled(true).setHeaderFormat(xFormatStr+" "+xLabel+" Range<br/>").setPointFormat("<span style=\"color:{point.color}\">\u25CF</span> {point.name} <b> Total: "+yFormatStr+" "+yLabel+"</b><br/>"))
        ...
        options.setxAxis(new Axis().setTitle(new Title(xLabel)).setCategories(categories));
