...
public class SimilarSearch extends FormTemplate {
    ...
    public SimilarSearch() {
        super(Constants.SIMILAR_SEARCH, getParams(), FormTemplate.similarityAssetSmall(), Arrays.asList(SimilarPatentServer.PATENTS_TO_SEARCH_FOR_FIELD,SimilarPatentServer.ASSIGNEES_TO_SEARCH_FOR_FIELD,SimilarPatentServer.TECHNOLOGIES_TO_SEARCH_FOR_ARRAY_FIELD));
    }
    ...
    public List<FormTemplate> nestedForms() {
        return Arrays.asList(new SimilarPatentSearch(), new SimilarApplicationSearch(), new SimilarAssigneeSearch(), new KeywordSearch());
    }
}
