...
//
// Solver for nu-models.svm classification and regression
//
...
public class models.svm {
					...
					double Cp, double Cn)
	{
		...
		if (Cp==Cn)
			models.svm.info("nu = "+sum_alpha/(Cp*prob.l)+"\n");
					...
					double[] alpha, Solver.SolutionInfo si)
	{
		...
		models.svm.info("nu = "+sum_alpha/(param.C*l)+"\n");
	}
		...
		double Cp, double Cn)
	{
		...
		models.svm.info("obj = "+si.obj+", rho = "+si.rho+"\n");
		...
		models.svm.info("nSV = "+nSV+", nBSV = "+nBSV+"\n");
				  ...
				  double[] probAB)
	{
		...
		for (iter=0;iter<max_iter;iter++)
		{
			...
			{
				models.svm.info("Line search fails in two-class probability estimates\n");
				break;
			}
		}
		
		if (iter>=max_iter)
			models.svm.info("Reaching maximal iterations in two-class probability estimates\n");
	...
	private static void multiclass_probability(int k, double[][] r, double[] p)
	{
		...
		if (iter>=max_iter)
			models.svm.info("Exceeds max_iter in multiclass_prob\n");
	}
	...
	private static double svm_svr_probability(svm_problem prob, svm_parameter param)
	{
		...
		for(i=0;i<prob.l;i++)
			if (Math.abs(ymv[i]) > 5*std) 
				...
				mae+=Math.abs(ymv[i]);
		mae /= (prob.l-count);
		models.svm.info("Prob. model for test data: target value = predicted value + z,\nz: Laplace distribution e^(-|z|/sigma)/(2sigma),sigma="+mae+"\n");
		return mae;
	}
	...
	public static svm_model svm_train(svm_problem prob, svm_parameter param)
	{
		...
		{
			// regression or one-class-models.svm
			model.nr_class = 2;
			model.label = null;
			model.nSV = null;
		...
		else
		{
			...
			if(nr_class == 1) 
				models.svm.info("WARNING: training data in only one class. See README for details.\n");
		...
		try
		{
			while(true)
			{
				...
				if(cmd.startsWith("svm_type"))
				{
					...
					{
						System.err.print("unknown models.svm type.\n");
						return false;
					}
				}
	...
	public static String svm_check_parameter(svm_problem prob, svm_parameter param)
	{
		...
		return "unknown models.svm type";
