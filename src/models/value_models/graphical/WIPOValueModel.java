...
public class WIPOValueModel extends ValueAttr {
    ...
    public void init() {
       ...
       // final String valueVariableName = "gatherValue";
        ...
        List<Item> items = new ArrayList<>(DataSearcher.searchForAssets(attributes, filters, Constants.NAME, SortOrder.ASC, maxLimit, nestedMap,false)).stream().filter(item->Database.getGatherValueMap().containsKey(item.getName())).collect(Collectors.toList());
