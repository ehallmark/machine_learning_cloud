 ...
 */
public class ParagraphCPCIterator implements SequenceIterator<VocabWord> {
    ...
    public ParagraphCPCIterator(FileTextDataSetIterator iterator, int numEpochs, Map<String,Collection<CPC>> cpcMap, int maxSamples, boolean fullText) {
    ...
    private static Sequence<VocabWord> extractSequenceFromDocumentAndTokens(LabelledDocument document, List<String> tokens, Random random, int maxSamples, boolean fullText) {
        ...
        boolean cpcLabels = false;
        Sequence<VocabWord> sequence = new Sequence<>(words);
        List<VocabWord> labels = new ArrayList<>(tokens.size());
        if(cpcLabels) {
            tokens.forEach(token -> {
                VocabWord label = new VocabWord(1, token);
                    ...
                    for (int i = 0; i < finalNumEpochs; i++) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            List<String> cpcs = document.getLabels().stream().flatMap(asset -> cpcMap.getOrDefault(asset, Collections.emptyList()).stream())
                                    .map(cpc->cpc.getName()).distinct().collect(Collectors.toCollection(ArrayList::new));
