...
public class DeepCPC2VecEncodingModel extends AbstractCombinedSimilarityModel<ComputationGraph,DeepCPC2VecEncodingPipelineManager> {
    ...
    protected Map<String, ComputationGraph> buildNetworksForTraining() {
        ...
        ComputationGraphConfiguration.GraphBuilder conf = new NeuralNetConfiguration.Builder(NNOptimizer.defaultNetworkConfig())
                ...
                //.addLayer(String.valueOf(i+3), NNOptimizer.newDenseLayer(input1+hiddenLayerSize,hiddenLayerSize).build(), String.valueOf(i+2))
        ...
        conf = conf.addLayer(String.valueOf(i+1),eNorm.build(), String.valueOf(i));
        i+=increment;
        ...
        conf = conf.addLayer(String.valueOf(i+1),dNorm.build(), String.valueOf(i));
        i+=increment;
        ...
        //  hidden layers
        for(; i < t + numHiddenLayers*increment; i+=increment) {
            org.deeplearning4j.nn.conf.layers.Layer.Builder layer = NNOptimizer.newDenseLayer(hiddenLayerSize,hiddenLayerSize);//i==t?(hiddenLayerSize+vectorSize):(hiddenLayerSize),hiddenLayerSize);
            org.deeplearning4j.nn.conf.layers.Layer.Builder norm = NNOptimizer.newBatchNormLayer(hiddenLayerSize,hiddenLayerSize);
            conf = conf.addLayer(String.valueOf(i),layer.build(), String.valueOf(i-1));
            conf = conf.addLayer(String.valueOf(i+1),norm.build(), String.valueOf(i));
        }
        ...
        // output layers
        OutputLayer.Builder outputLayer = NNOptimizer.newOutputLayer(hiddenLayerSize,input1).lossFunction(lossFunction);
        OutputLayer.Builder dateLayer = NNOptimizer.newOutputLayer(hiddenLayerSize,1).lossFunction(LossFunctions.LossFunction.MSE);

        conf = conf.addLayer("y1",outputLayer.build(), String.valueOf(i-1));
        conf = conf.addLayer("y2",dateLayer.build(), String.valueOf(i-1));
