...
public class PostgresVectorizedSequenceIterator implements SequenceIterator<VocabWord> {
    ...
    private final int vectorIdx;
    @Getter
    private Number[] vector;
    public PostgresVectorizedSequenceIterator(PreparedStatement ps, int textIdx, int vectorIdx, long... limits) {
        this.ps=ps;
        this.limits=limits;
        this.iter=0;
    ...
    public Sequence<VocabWord> nextSequence() {
        try {
            ...
            vector = (Number[]) rs.getArray(vectorIdx).getArray();
