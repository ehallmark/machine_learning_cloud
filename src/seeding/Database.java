...
public class Database {
	...
	private static final String patentVectorStatement = "SELECT pub_doc_number, regexp_split_to_array(abstract, '[_-\\.\\s+]'), regexp_split_to_array(substring(description FROM 1 FOR ?), '[_-\\.\\s+]') FROM patent_grant WHERE pub_date >= ? AND (abstract IS NOT NULL AND description IS NOT NULL)";
