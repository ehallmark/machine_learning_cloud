...
public class Database {
	...
	public static void insertBOW(String name, Integer[] values) throws SQLException {
		PreparedStatement ps = insertConn.prepareStatement("insert into bag_of_words (name,bow) values (?,?) on conflict (name) do update set bag_of_words.bow=array_agg(unnest(?)+unnest(bow)) where bag_of_words.name=?");
