...
public class Database {
	...
	private static final String patentVectorWithTitleAndDateStatement = "SELECT pub_doc_number, pub_date, invention_title, abstract, substring(description FROM 1 FOR ?) FROM patent_grant as p join patent_grant_uspto_classification as q on (p.pub_doc_number=q.pub_doc_number) WHERE pub_date >= ? AND (abstract IS NOT NULL and description IS NOT NULL AND invention_title IS NOT NULL)";
