...
public class UpdateDefaultCandidateSet {
    public static void main(String[] args) throws Exception {
        ...
        List<Patent> patentList = new SimilarPatentFinder(lookupTable).getPatentList();
            ...
            if(!cache.containsKey(name)) {
                ...
                if(current.size()>=chunkSize) {
                    ...
                    while (inner.next()) {
                        ...
                        try {
                            INDArray array = SimilarPatentFinder.handleResultSet(name,lookupTable);
                            if (array != null) {
        ...
        if(current.size()>0) {
            ...
            while (inner.next()) {
                try {
                    INDArray array = SimilarPatentFinder.handleResultSet(rs.getString(1),lookupTable);
                    if (array != null) {
                        patentList.add(new Patent(inner.getString(1), array));
                    }
                } catch (Exception e) {
