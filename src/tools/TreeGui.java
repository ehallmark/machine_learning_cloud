    ...
    public Triple<Integer,Integer,Integer> RenderTree(Graphics g, int fontSize, int StartWidth, int EndWidth, int StartHeight, int Level, TreeNode<T> node) {
        ...
        if(!node.getChildren().isEmpty()) {
            ...
            for (TreeNode<T> child : node.getChildren()) {
                ...
                if (childCoords != null) {
                    // draw lines
                    g.drawLine(childCoords.getFirst()+(childCoords.getThird()/2), childCoords.getSecond()-2*lines.length*fontSize, coords.getFirst()+(maxDataWidth/2), coords.getSecond()+2*lines.length*fontSize);
                }
                idx++;
            }
        }
